...
def breadthFirstSearch(gameState):
    ...
    while frontier:
        node = frontier.popleft()  # lấy state đầu tiên
        node_action = actions.popleft()  # lấy action đầu tiên
        if isEndState(node[-1][-1]):  # nếu các box vào hết vô goal
            temp += node_action[1:]  # thì lấy các action
            break
        if node[-1] not in exploredSet:  # nếu trạng thái chưa được duyệt
            exploredSet.add(node[-1])   # trạng thái đã duyệt
            # lấy các action của trạng thái đang duyệt
            for action in legalActions(node[-1][0], node[-1][1]):
                newPosPlayer, newPosBox = updateState(
                    node[-1][0], node[-1][1], action)  # cập nhật trạng thái mới với action
                if isFailed(newPosBox):  # xem vị trí của box có bị dính tường hay không
                    continue
                # thêm trạng thái vào cuối frontier
                frontier.append(node + [(newPosPlayer, newPosBox)])
                # thêm action vào cuối actions
                actions.append(node_action + [action[-1]])
    return temp
...
def uniformCostSearch(gameState):
    ...
    while not frontier.isEmpty():
        node = frontier.pop()  # lấy state có chi phí nhỏ nhất ứng với action
        node_action = actions.pop()  # lấy action có chi phí nhỏ nhất
        if isEndState(node[-1][-1]):  # nếu các box vào hết vô goal
            temp += node_action[1:]  # thì lấy các action
            break
        if node[-1] not in exploredSet:  # nếu trạng thái chưa được duyệt
            ...
            for action in legalActions(node[-1][0], node[-1][1]):
                ...
                if isFailed(newPosBox):  # xem vị trí của box có bị dính tường hay không
                    continue
                # thêm trạng thái vào frontier với chi phí tương ứng với action
                costaction = cost(action[-1])+cost(node_action)
                frontier.push(
                    node + [(newPosPlayer, newPosBox)], costaction)
                # thêm action vào actions với chi phí được tính bởi hàm cost
                actions.push(
                    node_action + [action[-1]], costaction)
    return temp
